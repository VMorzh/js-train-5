...
// Завдання 4: Дано масив рядків. За допомогою циклу for переберіть масив, видаліть
//третій елемент масиву та перетворіть кожен елемент в рядок за допомогою методу toString(). Результат виведіть на консоль.
...
function printUppercase(array) {
  ...
  for (var i = 0; i < array.length; i++) {
    var uppercaseElement = array[i].toUpperCase();
...
// Перетворіть масив в рядок за допомогою методу toString() та повторіть його 2 рази.
//Результат виведіть на консоль.
function printStrings(array) {
  const comboString = array.toString().repeat(2);
...
// За допомогою методу toString() перетворіть масив в рядок,
//та виясніть чи є в цьому рядку підрядок subString .

function concatenateStrings(array, subString) {
  const mainString = array.toString();
...
function swapMinMax(array) {
  ...
  if (array.length < 2) {
    console.log("Масив повинен містити щонайменше два елементи.");
    return;
  }
...
function replaceNumbers(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] > 10) {
      replacedArray.push("Greater than 10");
    } else {
      replacedArray.push("Less than or equal to 10");
    }
  }
